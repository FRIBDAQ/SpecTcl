...
proc CreateGateGenerator {parent} {
	...
	frame $wname -borderwidth 2 -relief groove -background lightyellow
		...
		tk_optionMenu $wname.r.t.operation GateOperation AND OR NOT C2BAND
		$wname.r.t.operation configure -background lightyellow
		entry $wname.r.b.depend -width 30 -textvariable GateDependency -background lightyellow
	...
	frame $wname -borderwidth 2 -relief groove -background lightblue
		button $wname.delete -text Delete -command DeleteGate -background lightblue
	...
	mclistbox::mclistbox $lbox -fillcolumn name -height 15 -width 60 \
	...
	$lbox column add depend -label Dependency -width 50
...
proc DeleteGate {} {
	...
	foreach index $selectedlist {
		...
		SendMessage "gate -delete [lindex $gate 0]"
		GetResponse
...
proc DeleteGateAll {} {
	...
	set lbox $topg.glist.listbox
	SendMessage "gate -delete [$lbox.framename.listbox get 0 end]"
	GetResponse
	UpdateGateList
}
...
proc CreateGate {} {
	...
	switch -- $GateOperation {
		AND {
			...
			SendMessage "gate $GateName * {$GateDependency}"
			GetResponse
		}
		OR {
			if {[llength $GateDependency] < 2} {
		...
		C2BAND {
			...
			SendMessage "gate $GateName c2band {$GateDependency}"
			GetResponse
		}
	}
	UpdateGateList
