/*!
   This software is a part of the ORNL Radware package by David Radford
   and is redistributed with permission.
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#else
      printf(".his files not supported;"
...
int dospkio(int mode, FILE *spkfile, int *idat, int ia, int nw)
{
  ...
  if (mode == 1) {
    if (fread(idat, 2*nw, 1, spkfile) != 1) {
      printf("ERROR - cannot read from spk file.\n");
      return 1;
    }
  } else {
    if (fwrite(idat, 2*nw, 1, spkfile) != 1) {
      printf("ERROR - cannot write to spk file.\n");
      return 1;
    }
  }
  return 0;
...
#define maxc ihed2[16]
  ...
  if (mode == 0) {   /* initialize */
    ...
    if (fwrite(idir, lrecl, 1, spkfile) != 1) {
      printf("SPKIO ERROR - write failure for directory.\n");
      return 4;
    }
    return 0;
  }
  ...
  if (dospkio(1, spkfile, idir, 1, 1024)) {
    printf("SPKIO ERROR - read failure for directory.\n");
    return 4;
  }
  ...
  if (mode == 6) {   /* return id-list in idat (idat[0] = # of id's) */
    ...
    if ((n = locateID(idir, 1, nid, idn)) <= 0) {
      printf("SPKIO ERROR - no such spectrum ID!\n");
      return 1;
    }
    nwn = idir[n+1];
    loch = nwn;
    ...
    if (dospkio(1, spkfile, ihed2, nwn, 64)) {
      printf("SPKIO ERROR - cannot read header(1)\n");
      printf("%d %d\n", nwn, 64);
     return 4;
    }
    ...
    if (dospkio(1, spkfile, ihed, nwn, nwd)) {
      printf("SPKIO ERROR - cannot read header(2)\n");
      printf("%d %d\n", nwn, nwd);
      return 4;
    }
    ...
    if (ihed[7] >= 0) {  /*+++ old spk-file form */
      ...
      printf("SPKIO ERROR - illegal ndx = %d   lenc :%d\n", *ndx, lenc);
      return 2;
    }
    ...
    if (dospkio(1, spkfile, &idat[idx], nwn, 2*nc)) {
      printf("SPKIO ERROR - cannot read spectrum\n");
      printf("%d %d \n", nwn, 2*nc);
      return 4;
    }
    return 0;
	...
	(n = locateID(idir, 1, nid, id)) != 0) { /* id exists */
      printf("SPKIO ERROR - spectrum ID already exists!\n");
      return 7;
    }
...
FILE *spkman(char *mode, char *namf, char *iacp)
{
  ...
  if (strcmp(cnam+iext, ".SPK") &&   /* test for .SPK ext */
    ...
    printf("SPKMAN ERROR - filename extension must be .spk!\n");

  } else if (!strncmp(mode, "CREA", 4)) {
    /* create a new spk-file and open for output */
    if (!(file = fopen(cnam, "w+"))) {
      printf("SPKMAN ERROR - cannot create new file %s\n", cnam);
    }
    /* init spk-file */
    else if (spkio(0, file, 0, 0, 0, 0, 0, 0)) {
      printf("SPKMAN ERROR - cannot initialize file %s\n", cnam);
      fclose(file);
      return NULL;
    }
    ...
    if (!(file = fopen(cnam, "r+")))
      printf("SPKMAN ERROR - cannot open (read/write) file %s\n", cnam);

  } else {
    if (!(file = fopen(cnam, "r")))
      printf("SPKMAN ERROR - cannot open (readonly) file %s\n", cnam);
  }
  return file;
} /* spkman */
#undef lrecl
...
int spkread(char *fn, float *sp, char *namesp, int *numch, int idimsp)
{
  ...
  if (!inin(fn, strlen(fn), &id, &in2, &in3)) {
    ...
    if (caskyn("Display directory? (Y/N)")) {
      /* list file directory */
      if (spkio(6, spkfile, id, ihed, maxh, isav, ndx, 0)) goto READERR;
      ns = isav[0];
      if (isav[ns] - isav[1] == ns - 1) {
	printf("%d spectra, numbers %d through %d\n", ns, isav[1], isav[ns]);
      } else {
	printf("%d spectra, numbers:\n", ns);
   ...
   /* ask for spectrum id */
    while ((nc = cask("Spectrum ID = ?", ans, 80)) &&
