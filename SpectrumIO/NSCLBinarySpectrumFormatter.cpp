...
"(C) Copyright 1999 NSCL, All rights reserved NSCLBinarySpectrumFormatter.cpp \n";
					      ...
					      ParameterDictionary& rDict)  
{
  ...
  CSpectrum* pSpectrum;
  pSpectrum = Factory.CreateSpectrum(Name, eSpecType, eDataType, vParameters, vResolutions);
  pSpectrum->Clear();
...
insertdata(CSpectrum *spectrum, vector<long>& Channels, const vector<UInt_t>& vDimensions, const vector<UInt_t>& vResolutions) {
  UInt_t channelarray[vDimensions.size()];
...
goodchannel(UInt_t* channelarray,  const vector<UInt_t>& vDimensions, const vector<UInt_t>& vResolutions) {
  ...
  for(int i = 0; i<vDimensions.size();i++) {
    ...
    if (channelarray[i]>=f2n(vResolutions[i])) {
      isgood=0;
      break;
    }
