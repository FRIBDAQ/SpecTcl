...
**     If there are no fonts that will fit, then the string is truncated
**     bit by bit (with a trailing elpisis) until it either fits or
**     only the elipsis is left at which point it is not drawn.
**
** Formal Parameters:
**    Display *display:
...
void Xamine_DrawFittedString(Display *display, Drawable drawable,
			     ...
			     char *label)
{
  char* string = new char[strlen(label) + 1];
  strcpy(string, label);
  ...
  */
  if(fonts == NULL) 
    LoadFonts(display);
  /*
  ** Select the font to use... trimming the string if needed
  ** until one is found.
  */
  ...
  while (font == NULL) {
    ...
    if (size < 5) return;       // not enough chars to make it worthwhile.
    string[size-1] = 0;		// Shrink the string...
    string[size-2] = '.';
    string[size-3] = '.';
    string[size-4] = '.';       // Fill tail with elipsis.

    font = fonts->SelectFont(string, (xh-xl), (yl - yh));
  }
