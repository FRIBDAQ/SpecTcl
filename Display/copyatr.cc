...
** Local storage:
*/
static const char *mode[] = { "Sampled",
			"Summed",
			"Averaged"
		      };
static const char *rend2[] = {
                         "Scatter",
			 "Boxes",
			 "Color",
			 "Contour",
...
{
 public:
  AttributeCopyDialog(const char *name, XMWidget *parent, const char *title,
		      win_attributed *atr);
  ~AttributeCopyDialog();
...
AxisLabels::AxisLabels(XMWidget *parent, win_attributed *atr) :
  CheckList(const_cast<char*>("AxisLabels"), parent, 
	    const_cast<char*>("Axis Labels"), 3)
{
...
*/
AttributeCopyDialog::AttributeCopyDialog(const char *name, XMWidget *parent, 
					 const char *title,
					 win_attributed *atr) :
