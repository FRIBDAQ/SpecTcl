...
static char *sccsinfo = "@(#)spcdisplay.cc	2.1 12/22/93 \n"; 
...
void Xamine_MultiplySelectedScale(float multiplier)
{
  ...
  if(at->manuallyscaled()) {
    unsigned int scale = at->getfs();
    scale = (int)((float)scale*multiplier);
    if(scale == 0) scale = 1;	/* Don't allow scale to go to zero. */
    if( (scale < at->getfs()) && (multiplier > 1.0))
      scale = at->getfs();      /* Don't let scale wrap. */
    at->setfs(scale);
