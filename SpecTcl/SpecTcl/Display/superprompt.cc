...
void SuperPrompt::SetList()
{
  ...
  if( (tgttype != onedlong) &&
      (tgttype != onedword)) {
    Xamine_error_msg(this, "Selected spectrum not 1-d");
    Hide();
    return;
  }
  ...
  */
  if(slots_filled == 0) {
    Xamine_error_msg(this, "There are no compatible spectra to superimpose");
    Hide();
  }
  else {
...
void Xamine_GetSuperposition(XMWidget *parent, XtPointer u, XtPointer c)
{
  ...
  if(dialog == NULL) {
    dialog = new SuperPrompt("Superpose", *parent);
  }
...
{
  if(!undialog) {
    undialog = new UnSuperPrompt("UnSuperimpose", *wid);
  }
  undialog->SetupList();
  undialog->Manage();
}
