...
/** Modified for use as the SpecTcl shell.
 */
...
#endif

extern int Spectclexperiment_Init(Tcl_Interp* pInterp);
/*
 *----------------------------------------------------------------------
 *
 * main --
    ...
    Tcl_Interp *interp)		/* Interpreter for application. */
{
    const char* pVersion;
    int         status;
    Tcl_DString scriptPath;
    const char*       scriptPathElements[] = {
	LIBDIR,
	"spectcl.tcl"
    };
    ...
    status = Spectclexperiment_Init(interp);
    if (status != TCL_OK) {
	fprintf(stderr, "Failed to init SpecTcl package : %s",
		Tcl_GetStringResult(interp));
    }
    pVersion = Tcl_PkgRequire(interp, "SpecTcl","1.0", 0);
    if (!pVersion) {
	fprintf(stderr, "Failed load of SpecTcl package\n");
	const char* pMsg = Tcl_GetStringResult(interp);
	if (pMsg) {
	    fprintf(stderr, "%s\n", pMsg);
	}
	return TCL_ERROR;
	...
	fprintf(stderr, "Loaded SpecTcl %s\n", pVersion);
    }

    /* Source in the SpecTcl shell definitions.  */

    Tcl_DStringInit(&scriptPath);
    pScriptPath = Tcl_JoinPath(2, scriptPathElements, &scriptPath);
    status      = Tcl_EvalFile(interp, pScriptPath);
    if (status != TCL_OK) {
	fprintf(stderr, "Failed to define shell procs: %s\n", 
		Tcl_GetStringResult(interp));
	return TCL_ERROR;
    }
