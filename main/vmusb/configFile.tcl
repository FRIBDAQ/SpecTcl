...
#---------------------------------------------------------------
# The mqdc command processes the creation and configuration of
# Mesytec 32 channel QDC modulees.
#
proc mqdc args {
    set subcommand [lindex $args 0]
    set name       [lindex $args 1]

    set ::readoutDeviceType($name) $::typeMQDC32
	...
	# Save the -resolution parameter in mqdcResolutions(name)
	
	set residx [lsearch -exact $args "-resolution"]
	if {$residx != -1} {
	    incr residx
	    set ::mqdcResolutions($name) [lindex $args $residx]
	}
    }
}
      ...
      if {$outputformatidx != -1} {
        ...
        if {$outputformat == 4} {
          set ::readoutDeviceType($name) $::typeMDPP32SCPSRO
				...
				if {$outputformat == 8 && $::MDPPSCPSROSoftTrigger == 1} {
