 ...
 */
int TreeCommand::operator()(CTCLInterpreter &interp,
 ...
 */
void TreeCommand::create(CTCLInterpreter &interp,
                         std::vector<CTCLObject> &objv) {
  SpecTcl *pApi = SpecTcl::getInstance();

  requireAtLeast(
      objv, 4,
      "roottree create requires at least a name and parameter pattern list");
  requireAtMost(objv, 5,
                "roottree create can at most have a name, parameter "
                "patternlist and gate name");
  ...
  bool foundOne(false);
  for (auto p = pApi->BeginParameters();
       (p != pApi->EndParameters()) && !foundOne; p++) {
  ...
  if (!foundOne) {
    interp.setResult(
        "*Warning* - parameter patterns don't match any parameters");
  }
 ...
 *   exceptions.
 */
void TreeCommand::destroy(CTCLInterpreter &interp,
                          std::vector<CTCLObject> &objv) {
 ...
 */
void TreeCommand::list(CTCLInterpreter &interp, std::vector<CTCLObject> &objv) {
  requireAtMost(objv, 3,
                "roottree list - can have at most a tree name pattern");
  ...
  CTCLObject result;
  result.Bind(interp);
  for (auto p = api->EventSinkPipelineBegin(); p != api->EventSinkPipelineEnd();
       ++p) {
    auto name = p->first;
    CEventSink *pRawItem = p->second;
    RootTreeSink *pSink = dynamic_cast<RootTreeSink *>(pRawItem);

    if (pSink && Tcl_StringMatch(name.c_str(),
                                 name.c_str())) { // It's a matching tree sink.
      CTCLObject entry;
      entry.Bind(interp);
...
/**
 * setdir
 *    Set the directory in which the tree files are created.
 */
void TreeCommand::setdir(CTCLInterpreter &interp,
                         std::vector<CTCLObject> &objv) {
  requireExactly(objv, 4, "roottree setdir requires a tree name and a path");
  std::string treeName = objv[2];
  std::string path = objv[3];

  auto pSink = findSink(sinkName(treeName).c_str());
  if (!pSink) {
    std::stringstream s;
    s << "There is no Root tree event for " << treeName;
    std::string msg(s.str());
 ...
 */
std::string TreeCommand::sinkName(std::string treeName) const {
...
/**
 *  findSink
 *    Return a pointer to the RootTreeSink that coresponds to the given sink
 * name. Note that more than one sink can have the same name.  We return the
 * first one we find that has the right name and dynmamically casts to a
 * RootTreeSink.
 *
 ...
 */
RootTreeSink *TreeCommand::findSink(const char *pName) const {
  auto api = SpecTcl::getInstance();
  for (auto p = api->EventSinkPipelineBegin(); p != api->EventSinkPipelineEnd();
       ++p) {
    if (p->first == pName) {
