...

/*
   Lesstif has a nasty little file dialog bug we're going to 'sort of' work  
   around this.  In lesstif, selecting the file corrupts the filter string to
   be the file that was selected.  We're going to get the directory of the
   selected file, pop a *.win on the back end and re-set the search string
   from that.

   parameters:
      XMFileListDialog* widget.
    
*/
void
resetSearchMask(XMFileListDialog* widget)
{
  string currentMask(widget->GetFullSearchString());
  char  *pMask   = new char[strlen(currentMask.c_str()) +1];
  strcpy(pMask, currentMask.c_str());
  string searchDir(dirname(pMask));
  delete []pMask;

  searchDir += "/*.win";
  widget->DoSearch(searchDir.c_str());
}
