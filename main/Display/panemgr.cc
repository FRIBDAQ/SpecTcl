...
std::shared_ptr<Win::SpectrumQuerier> gSpectrumQuerier(new XamineSpectrumQuerier);

/*
**++
...
pane_db::pane_db(XMForm *parent, win_title title) :
    win_db(1,1, title) {        

      gSpectrumInterface.setQueryEntity(gSpectrumQuerier);

      init_panedb(parent, title);
      status_bar  = True;
    }
...
void pane_db::unzoom()
{
   ...
   // Not sure why these are here, but they are clearly dead code.
   //  height = height / ny() - 1;
   //  width  = width  / nx() - 1;
...
void Xamine_SelectPane(int row, int col)
{
  ...
  /* Set the state of the mapping button */
  if(att != (win_attributed*)NULL) {
    win_1d* a1;
    win_2d* a2;
    if(att->is1d()) {
      a1 = (win_1d*) att;
      Xamine_SetButtonBarApplyMap(a1->ismapped());
    }
    else {
      a2 = (win_2d*) att;
      Xamine_SetButtonBarApplyMap(a2->ismapped());
    }
  }
...
void Xamine_SetSelectedDisplay(int spno)
{
  ...
  Xamine_SetDisplay(row, col, spno);
}
...
void Xamine_SetDisplayAttributes(int row, int col, win_attributed *attribs)
{
  ...
  if(Xamine_panedb->defined(col, row)) {
	...
	def1->expand(at1->lowlimit(), at1->highlimit());
      if(at1->ismapped())       /* Copy the mapped state.. */
	def1->setmapped(TRUE);
	...
	def2->expand(at2->xlowlim(), at2->xhilim(),
      ...
      if(at2->ismapped())      /* Copy the mapped state */
	def2->setmapped(TRUE);
    }
  }
  else {
