...
/** @file:  attribdefault.cc
 *  @brief: attribute settings.
 */
...
static void ApplyDefaultsEverywhere(win_attributed *def)
{
  ...
  for(int r = 0; r < rows; r++) {
	...
	/* If the mapped min and max are both zero, don't map! */
	if(xamine_shared->getxmax_map(a->spectrum()) != 
	   xamine_shared->getxmin_map(a->spectrum()))
	  a->setmapped(def->ismapped());  /* Set the user mapping state. */
