...
{
  if(state) mapping_button->Set();
  else      mapping_button->UnSet();
}
...
{
  return mapping_button->GetState();
}

void Xamine_SetApplyMapSensitivity(Boolean b)
{
  if(b) mapping_button->SetAttribute(XmNsensitive, (long int)True);
...
void Xamine_SetupButtonBar(XMWidget *button_bar)
{
  ...
  windows_rc->SetLeftPosition(*tb, 22);
  ...
  spectra_rc->SetRightAttachment(*mapping_button,  XmATTACH_NONE);
  spectra_rc->SetBottomAttachment(*mapping_button, XmATTACH_FORM);
  spectra_rc->SetTopAttachment(*mapping_button, XmATTACH_WIDGET);
  spectra_rc->SetTopWidget(*mapping_button, *pbt2);
  Xamine_AddtoSpectrumSelectedPackage(mapping_button);
