...
*/
static const char *yaccrevlevel = "@(#)windfile.y	8.1 6/23/95  - windows file parser\n";
...
Win::SpectrumQueryResults getSpectrumInfo();
enum IdentifierType { INT, STRING } gSpectrumIdType;
static int x,y;
static int specnum;
...
window_clause: WINDOW INTEGER COMMA INTEGER COMMA spectrum blankline
{
  ...
  if (gSpectrumIdType == INT) {                 
    setUpDbEntry(specnum, $2, $4);
  } // end integer definition
  else {
    setUpDbEntry(specname, $2, $4);
  } // end name definition
...
expanded_clause:  EXPANDED xlimit  ylimit blankline
          ...
          Win::SpectrumQueryResults info;
          info = getSpectrumInfo();
		      int topx = info.s_nBinsX;
		      int topy = info.s_nBinsY;
...
static int superimposable(int target, int spectrum)
{
  ...
  targetInfo = gSpectrumInterface.getSpectrumInfo(target);

  if( ! targetInfo.s_exists ) return 0;

  specInfo = gSpectrumInterface.getSpectrumInfo(spectrum);
  
  if(targetInfo.s_nBinsX != specInfo.s_nBinsY) {
    return -1;
  }
...
Win::SpectrumQueryResults getSpectrumInfo() {
  if (gSpectrumIdType == INT) {
    return gSpectrumInterface.getSpectrumInfo(specnum);
  } else {
    return gSpectrumInterface.getSpectrumInfo(specname);
  }
}
